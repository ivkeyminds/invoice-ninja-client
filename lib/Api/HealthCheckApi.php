...
class HealthCheckApi
{
     ...
     * @return void
     */
    public function getHealthCheck($x_requested_with, string $contentType = self::contentTypes['getHealthCheck'][0])
    {
        $this->getHealthCheckWithHttpInfo($x_requested_with, $contentType);
    }
     ...
     */
    public function getHealthCheckWithHttpInfo($x_requested_with, string $contentType = self::contentTypes['getHealthCheck'][0])
    {
        $request = $this->getHealthCheckRequest($x_requested_with, $contentType);
     ...
     */
    public function getHealthCheckAsync($x_requested_with, string $contentType = self::contentTypes['getHealthCheck'][0])
    {
        return $this->getHealthCheckAsyncWithHttpInfo($x_requested_with, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function getHealthCheckAsyncWithHttpInfo($x_requested_with, string $contentType = self::contentTypes['getHealthCheck'][0])
    {
        $returnType = '';
        $request = $this->getHealthCheckRequest($x_requested_with, $contentType);
     ...
     */
    public function getHealthCheckRequest($x_requested_with, string $contentType = self::contentTypes['getHealthCheck'][0])
    {
        ...
        if ($x_requested_with === null || (is_array($x_requested_with) && count($x_requested_with) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $x_requested_with when calling getHealthCheck'
            );
        }


        $resourcePath = '/api/v1/health_check';
        $formParams = [];
        $queryParams = [];
        $headerParams = [];
        $httpBody = '';
