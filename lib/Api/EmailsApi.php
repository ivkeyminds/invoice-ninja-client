...
class EmailsApi
{
     ...
     * @param  \IvkeyMinds\InvoiceNinjaClient\Model\V1EmailsBody $v1_emails_body The template subject and body (required)
     ...
     */
    public function sendEmailTemplate($x_requested_with, $v1_emails_body, string $contentType = self::contentTypes['sendEmailTemplate'][0])
    {
        list($response) = $this->sendEmailTemplateWithHttpInfo($x_requested_with, $v1_emails_body, $contentType);
        return $response;
    }
     ...
     */
    public function sendEmailTemplateWithHttpInfo($x_requested_with, $v1_emails_body, string $contentType = self::contentTypes['sendEmailTemplate'][0])
    {
        $request = $this->sendEmailTemplateRequest($x_requested_with, $v1_emails_body, $contentType);
     ...
     */
    public function sendEmailTemplateAsync($x_requested_with, $v1_emails_body, string $contentType = self::contentTypes['sendEmailTemplate'][0])
    {
        return $this->sendEmailTemplateAsyncWithHttpInfo($x_requested_with, $v1_emails_body, $contentType)
            ->then(
                function ($response) {
                    return $response[0];
                }
     ...
     * @return \GuzzleHttp\Promise\PromiseInterface
     */
    public function sendEmailTemplateAsyncWithHttpInfo($x_requested_with, $v1_emails_body, string $contentType = self::contentTypes['sendEmailTemplate'][0])
    {
        $returnType = '\IvkeyMinds\InvoiceNinjaClient\Model\Template';
        $request = $this->sendEmailTemplateRequest($x_requested_with, $v1_emails_body, $contentType);
     ...
     */
    public function sendEmailTemplateRequest($x_requested_with, $v1_emails_body, string $contentType = self::contentTypes['sendEmailTemplate'][0])
    {
        ...
        if ($x_requested_with === null || (is_array($x_requested_with) && count($x_requested_with) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $x_requested_with when calling sendEmailTemplate'
            );
        }

        // verify the required parameter 'v1_emails_body' is set
        if ($v1_emails_body === null || (is_array($v1_emails_body) && count($v1_emails_body) === 0)) {
            throw new \InvalidArgumentException(
                'Missing the required parameter $v1_emails_body when calling sendEmailTemplate'
            );
        }
        ...
        // for model (json/xml)
        if (isset($v1_emails_body)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false) {
                # if Content-Type contains "application/json", json_encode the body
                $httpBody = \IvkeyMinds\InvoiceNinjaClient\Utils::jsonEncode(ObjectSerializer::sanitizeForSerialization($v1_emails_body));
            } else {
                $httpBody = $v1_emails_body;
            }
